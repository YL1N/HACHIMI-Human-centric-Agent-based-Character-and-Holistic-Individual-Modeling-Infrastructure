...
class RateLimiter:
    ...
    def __init__(self, qpm_init: int = 6, capacity_max: int = 180):
        ...
        self.additive_every = 20  # 连续成功 S 次后 +1
        self.beta = 0.7           # 429/503 后乘性降低
...
def call_llm(messages: List[Dict[str, Any]], max_tokens=900, temperature=0.95) -> str:
    ...
    if client is None:
        # 回退到单 key（兼容旧逻辑）
        r = requests.post(f"{AIECNU_BASE_URL.rstrip('/')}{path}", headers=HEADERS, json=payload, timeout=120)
        r.raise_for_status()
        data = r.json()
    else:
        data = client.post(path, payload, timeout=120)
...
def _append_record_by_sid(run_dir: str, chunk_size: int, record: Dict[str, Any], write_lock: threading.Lock):
...
with st.sidebar:
    ...
    api_keys_text = st.text_area(
        "API Keys（每行一个；留空则使用默认单 key）",
        value=AIECNU_API_KEY
    )
    qpm_per_key = st.number_input("每个 Key 的 QPM（每分钟请求数）", min_value=1, value=10, step=1)
    per_step_m = st.number_input("每步并发生成条数（建议 ≤ Key 数×1）", min_value=1, value=4, step=1)
...
if "pending" not in st.session_state: st.session_state.pending = deque()        # 只存 sid
...
if start_btn:
    ...
    # —— 关键：一次性配表 schedule（sid → slot），并落盘
    schedule = []
    for sid in range(1, st.session_state.total_n + 1):
        slot = st.session_state.quota.next_slot() if st.session_state.quota and st.session_state.quota.has_next() else {}
        schedule.append({"sid": sid, "slot": slot})
    st.session_state.schedule = schedule
    st.session_state.sid2slot = {it["sid"]: it["slot"] for it in schedule}
    _save_schedule(st.session_state.run_dir, schedule)
    _save_meta(st.session_state.run_dir, st.session_state.total_n, st.session_state.chunk_size)

    # pending = 1..N；completed 清空；next_sid=1
    st.session_state.pending = deque([it["sid"] for it in schedule])
    st.session_state.completed_sids = set()
...
if resume_btn and st.session_state.running:
    ...
    if st.session_state.run_dir:
        ...
        st.session_state.pending = deque([sid for sid in range(1, st.session_state.total_n+1) if sid not in done])
...
if st.session_state.running:
    ...
    if not st.session_state.paused:
        ...
        # === 准备本步的 sid 批次 ===
        # 并发度：每步从 pending 取前 K 个
        batch_size = int(per_step_m)
        scheduled_sids = []
        for _ in range(min(batch_size, len(st.session_state.pending))):
            scheduled_sids.append(st.session_state.pending.popleft())
        if not scheduled_sids:
            _st_rerun()

        # 准备 clients（自适应）
        api_keys = [k.strip() for k in api_keys_text.splitlines() if k.strip()]
        qpm_init_guess = int(qpm_per_key) if isinstance(qpm_per_key, int) else 6
        clients = [KeyClient(k, AIECNU_BASE_URL, qpm_init=qpm_init_guess) for k in api_keys] if api_keys else [
            KeyClient(AIECNU_API_KEY, AIECNU_BASE_URL, qpm_init=qpm_init_guess)]

        # === 提交并行任务 ===
        accepted = 0
        last_error = None

        with ThreadPoolExecutor(max_workers=max(1, min(4*len(clients), len(scheduled_sids)))) as ex:
            futures = {}
            for i, sid in enumerate(scheduled_sids):
                client = clients[i % len(clients)]
                slot = st.session_state.sid2slot.get(sid, {})
                futures[
                    ex.submit(_worker_process_one, st.session_state.orch, sid, slot, client, st.session_state.sim_gate,
                              MAX_RETRIES_PER_SLOT)] = sid
            ...
            for fut in as_completed(futures):
                ...
                if ok and item:
                    # —— 按 sid 计算分片写盘（并发安全）
                    _append_record_by_sid(st.session_state.run_dir, st.session_state.chunk_size, item,
                                          st.session_state.write_lock)
                    st.session_state.completed_sids.add(sid)
                    # 推进 next_sid 指针
                    while st.session_state.next_sid in st.session_state.completed_sids and st.session_state.next_sid <= st.session_state.total_n:
                        st.session_state.next_sid += 1
                    accepted += 1
                    ...
                    # —— 失败：回队尾重试（直至成功，无上限）
                    st.session_state.pending.append(sid)
